"""
Script de auditoría para detectar tablas no utilizadas, duplicadas u obsoletas
...
4. Detecta duplicaciones y tablas huérfanas
"""
...
def extract_tables_from_schema_doc(schema_path: Path) -> Dict[str, Dict]:
    ...
    for match in re.finditer(table_pattern, content, re.IGNORECASE | re.DOTALL):
        ...
        # Extraer columnas (primera palabra después de espacio/newline)
        columns = []
...
def find_model_usage_in_endpoints(endpoints_dir: Path, models: Dict) -> Dict[str, List[str]]:
    """Detecta qué modelos se usan en qué endpoints"""
...
def generate_audit_report(
    ...
    # Generar Markdown
    lines = [
        "# Reporte de Auditoría: Base de Datos y API",
        "",
        ...
        "## Resumen Ejecutivo",
        "",
        f"- [INFO] **Tablas en esquema BD:** {len(schema_tables)}",
        f"- [INFO] **Modelos SQLAlchemy:** {len(models)}",
        f"- [OK] **Tablas activas (usadas en endpoints):** {len(active_tables)}",
        f"- [ZOMBIE] **Modelos zombie (no usados):** {len(zombie_models)}",
        f"- [WARN] **Tablas huerfanas (sin modelo):** {len(orphan_tables)}",
        f"- [INFO] **Posibles duplicaciones:** {len(similar_pairs)}",
        "",
        "---",
        "",
    ...
    lines.extend([
        ...
        "",
        "ADVERTENCIA **ACCIÓN RECOMENDADA:** Revisar si estos modelos son necesarios o se pueden eliminar.",
        ""
    ])
    ...
    if zombie_models:
        for model_name, table_name in sorted(zombie_models):
            ...
            lines.extend([
                ...
                f"- **Recomendación:** Verificar si es legacy o si falta implementar endpoints",
                ""
            ])
    else:
        lines.append("OK No se encontraron modelos zombie. Todos los modelos están en uso.")
        lines.append("")
    
    lines.extend([
        "---",
        "",
        "## [3] Tablas HUERFANAS (En BD pero sin modelo)",
        "",
        "ADVERTENCIA **ACCIÓN RECOMENDADA:** Crear modelos o considerar eliminar estas tablas.",
        ""
    ])
    
    if orphan_tables:
        for table_name in sorted(orphan_tables):
            desc = schema_tables.get(table_name, {}).get('description', 'Sin descripción')
            lines.extend([
                f"### ADVERTENCIA `{table_name}`",
                "",
                f"- **Descripción:** {desc}",
                f"- **Estado:** Existe en BD pero NO tiene modelo SQLAlchemy",
                f"- **Recomendación:** Crear modelo si es necesaria, o eliminar si es legacy",
                ""
            ])
    else:
        lines.append("OK No se encontraron tablas huérfanas. Todas las tablas tienen modelos.")
        lines.append("")
    ...
    lines.extend([
        ...
        "",
        " Tablas con nombres similares que podrían estar duplicando funcionalidad.",
        ""
    ])
    ...
    if similar_pairs:
        for table1, table2, similarity in sorted(similar_pairs, key=lambda x: x[2], reverse=True):
            lines.extend([
                f"### [SIMILAR] `{table1}` <-> `{table2}` (similitud: {similarity:.0%})",
                "",
                ...
                f"- **Recomendación:** Revisar si pueden consolidarse",
                ""
            ])
    else:
    ...
    lines.extend([
        ...
        "2. **Crear modelos faltantes:** Para tablas huérfanas que se usen",
        ...
        "---",
        "",
        "**Reporte generado automáticamente por `audit_db_usage.py`**"
    ])
...
def main():
    """Función principal"""
    print(" Iniciando auditoría de Base de Datos y API...")
